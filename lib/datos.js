// Datos de la libreta en Redis, separados por cuenta.
//   libreta:u:<usuario>:mes:AAAA-MM    hash  id -> movimiento (JSON)
//   libreta:u:<usuario>:aprendidas     hash  palabra -> categoría
//   libreta:u:<usuario>:presupuestos   hash  categoría -> tope mensual en pesos
//   libreta:u:<usuario>:ultimo         JSON  último lote cargado por el bot (para "deshacer")
//   libreta:msg:<id>                   marca de mensaje del bot ya procesado (global)
...
// Devuelve las operaciones de datos de una cuenta.
function datosDe(usuario) {
  if (!/^[a-z0-9_.]{3,20}$/.test(usuario || '')) throw new Error('Usuario inválido');
  ...
  return { usuario, itemsMes, agregar, actualizar, borrar, ajustes, aprender, olvidar, fijarPresupuestos, guardarUltimo, deshacerUltimo };
}

// true si el mensaje es nuevo; false si la app de mensajería lo está reenviando.
async function marcarMensaje(id) {
  return (await cmd('SET', 'libreta:msg:' + id, '1', 'NX', 'EX', '172800')) === 'OK';
}

module.exports = { datosDe, limpiar, marcarMensaje, mesDe };
